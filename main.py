...
DATABASE_PATH = os.getenv("DATABASE_PATH", "data/pfp_theme_bot.db")
...
async def pfp_random(interaction: discord.Interaction):
    ...
    theme = db.pick_random_suggestion(int(active["id"]), THEMES)
    if not theme:
        await interaction.response.send_message(
            "There are no unused random themes available for this election.",
            ephemeral=True,
        )
        return

    added = db.add_suggestion(int(active["id"]), theme, interaction.user.id)
    if not added:
        await interaction.response.send_message(
            "I couldn't add that random theme. Please try **/pfp random** again.",
            ephemeral=True,
        )
        return
